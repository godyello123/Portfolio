...
#if SERVER_ONLY || MAIN_SERVER_ONLY || LOG_SERVER_ONLY
...
namespace SCommon
{
    public class SCSVReader
    {
        ...
        public Vector3 GetVector3(int row, string column)
        {
            if (!TryGetValue(row, column, out string text))
#if SERVER_ONLY || MAIN_SERVER_ONLY || LOG_SERVER_ONLY
                return Vector3.Zero;
#else
                return Vector3.zero;
        ...
        public Vector3 GetVector3(string text)
        {
            string[] value = text.Trim(' ', '\"').Trim('(', ')').Split(',');

#if SERVER_ONLY || MAIN_SERVER_ONLY || LOG_SERVER_ONLY
            Vector3 vector = Vector3.Zero;
            vector.X = (float)Convert.ChangeType(value[0], typeof(float));
            vector.Y = (float)Convert.ChangeType(value[1], typeof(float));
            vector.Z = (float)Convert.ChangeType(value[2], typeof(float));
#else
            Vector3 vector = Vector3.zero;
            vector.x = (float)Convert.ChangeType(value[0], typeof(float));
            vector.y = (float)Convert.ChangeType(value[1], typeof(float));
        ...
        public List<Vector3> GetListVector3(int row, string column, char delimiter = ',')
        {
            string stringValue = GetValue(row, column);
            if (string.IsNullOrEmpty(stringValue) == true)
            ...
            stringValue = stringValue.Trim('[', ']');
            var elements = stringValue.Split(delimiter);

            var resultList = new List<Vector3>();
            foreach (var element in elements)
            {
                Vector3 vector = GetVector3(element);
