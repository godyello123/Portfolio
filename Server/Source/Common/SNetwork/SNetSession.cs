...
namespace SNetwork
{
    public class SNetSession : IDisposable
    {
        private const int NETWORK_PACKETS_IN_PER_SEC = 30 * 5;
        private STimer m_NetworkPacketsInTimer = new STimer(5 * 1000, true);
        private int m_NetworkPacketsInPerSec = 0;

        public bool IsServerSession { get; set; } = false;

        public static bool s_SocketExceptionLog;

        public const int PACKET_HEADER = 2;
        public const int MAX_PACKET = ushort.MaxValue; //short.MaxValue + 1;
        public const int DEFAULT_RECV_BUFFER = ushort.MaxValue; //8192 * 4;
        public const int DEFAULT_SEND_BUFFER = ushort.MaxValue; //8192 * 4;
        ...
        private ArraySegment<byte> m_RecvMessage = SMemoryPool.Instance.Empty;
        private ushort m_RecvMsgLen = 0;
        ...
        public void Close(CloseType closeType = CloseType.Normal)
        {
            lock (this)
            {
                ...
                if (m_RecvMessage != SMemoryPool.Instance.Empty)
                    SMemoryPool.Instance.Return(m_RecvMessage);
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        while (true)
                        {
                            ...
                            byte[] copyBuff = new byte[length];
                            Buffer.BlockCopy(buff, offset, copyBuff, 0, length);
                            m_Session.GetStream().BeginWrite(copyBuff, 0, length, new AsyncCallback(OnSend), null);
        ...
        private void RecvMessage()
        {
            try
            {
                lock (this)
                {
                    ...
                    m_Session.GetStream().BeginRead(m_RecvMessage.Array, m_RecvMessage.Offset + m_Read,
                        m_RecvMsgLen - m_Read, new AsyncCallback(OnRecvMessage), null);
                }
            }
            catch (Exception e)
            {
        ...
        private void OnRecvHeader(IAsyncResult ar)
        {
            try
            {
                lock (this)
                {
                    ...
                    m_RecvMsgLen = (ushort)IPAddress.NetworkToHostOrder(BitConverter.ToInt16(m_RecvHeader, 0));
                    //ushort messageLength = (ushort)IPAddress.NetworkToHostOrder(BitConverter.ToInt16(m_RecvHeader, 0));
                    if (m_RecvMsgLen < 0 || m_RecvMsgLen > MAX_PACKET)
                    {
                        m_EventQueue.Push(new SNetEventException(m_SessionKey, new Exception(string.Format("MessageLength Error : Length[{0}]", m_RecvMsgLen))));
                        Close();
                        return;
                    }

                    m_RecvMessage = SMemoryPool.Instance.Rent(m_RecvMsgLen);
        ...
        private bool IsValidNetworkPacketsIn()
        {
            if (IsServerSession) return true;

            ++m_NetworkPacketsInPerSec;
            if (m_NetworkPacketsInTimer.Check())
            {
                if (m_NetworkPacketsInPerSec > NETWORK_PACKETS_IN_PER_SEC)
                    return false;

                m_NetworkPacketsInPerSec = 0;
            }
        ...
        private void OnRecvMessage(IAsyncResult ar)
        {
            try
            {
                lock (this)
                {
                    ...
                    m_Read += length;
                    if (m_Read < m_RecvMsgLen)
                    {
                        RecvMessage();
                        return;
                    }

                    if (!IsValidNetworkPacketsIn())
                    {
                        Close();
                        return;
                    }
                    ...
                    //m_EventQueue.Push(new SNetEventRecv(m_SessionKey, m_RecvMessage, m_RecvMsgLen));

                    //SArrayPool.Instance.Return(m_RecvMessage, false);

                    m_RecvMessage = SMemoryPool.Instance.Empty;
        ...
        public void Send(byte[] message, ushort length)
        {
            try
            {
                lock (this)
                {
                    if (DEFAULT_SEND_BUFFER < length)
                    {
                        m_EventQueue.Push(new SNetEventException(m_SessionKey, new Exception(string.Format("Send Error : Length[{0}]", length))));
                        return;
                    }
        ...
        public void Send(SNetWriter writer)
        {
            try
            {
                lock (this)
                {
                    ushort length = writer.GetBufferLength();
                    if (DEFAULT_SEND_BUFFER < length)
                    {
                        m_EventQueue.Push(new SNetEventException(m_SessionKey, new Exception(string.Format("Send Error : Length[{0}]", length))));
                        return;
                    ...
                    m_SendQueue.Push(writer.GetBuffer().Array, writer.GetBuffer().Offset, writer.GetBufferLength());
                }
            }
            catch (Exception e)
            {
    ...
    public class SNetSessionTable
    {
        ...
        public SNetSession Find(long sessionKey)
        {
        ...
        public void CheckSession()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (elapsed.TotalMilliseconds > SNetSystem.Timeout) 
                    session.Close(CloseType.Timeout);
            }
        }
        ...
        {
            SNetSession netSession = Find(sessionKey);
            if (netSession != null) netSession.Send(writer);
        }
